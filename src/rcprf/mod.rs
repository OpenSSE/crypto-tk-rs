...
/// Range structure and functions for use with range-constrained PRFs.
pub mod rcprf_range;
/// Traits used to describe range-constrained PRFs.
pub mod traits;

/// Nodes of the tree-based range-constrained PRFs.
pub(crate) mod inner_element;
/// Leaves of the tree-based range-constrained PRFs.
pub(crate) mod leaf_element;

/// All the generators for the range-constrained PRFs
pub mod iterator;

/// Key generation from range-constrained PRFs
...
/// Returns the maximum leaf index for a `RcPrf` using a tree of height
/// `height`. It returns 0 for a tree of height 0 and 2^64-1 for a `height`
/// larger or equal to `MAX_HEIGHT` (65)
#[must_use]
...
/// An *unconstrained* range PRFs object
#[derive(Zeroize)]
#[zeroize(drop)]
pub struct RcPrf {
...
/// A *constrained* range PRFs object (obtained after constraining a range PRF -
/// constrained or not)
...
impl ConstrainedRcPrf {
    ...
    /// Transform the constrained `RcPrf` into an iterator that produces pairs
    /// of index and evaluation value for that index.
    /// Values produced by that iterator are vectors of size `out_size`.
    #[must_use]
    pub fn into_value_iter(self, out_size: usize) -> iterator::RcPrfIterator {
    ...
    fn rcprf_constrain_consistency() {
        ...
        for start in 0..=max_leaf_index(h) {
            for end in start..=max_leaf_index(h) {
                ...
                let par_eval_res: Vec<(u64, Vec<u8>)> =
                    rcprf.value_range_par_iter(&range, 16).unwrap().collect();
                ...
                let triplets = direct_eval
                    ...
                    .zip(rcprf.value_range_iter(&range, 16).unwrap())
                ...
                let rev_couple = direct_eval
                    ...
                    .take(range_width)
                    .rev()
                    .zip(rcprf.value_range_iter(&range, 16).unwrap().rev());
